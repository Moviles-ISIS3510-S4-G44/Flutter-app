...
class AuthService {
  ...
  Future<SignupResponseDto> signup(SignupDto dto) async {
    try {
      ...
      if (response.statusCode == 400) throw Exception(data['detail'] ?? 'Invalid signup data');
      if (response.statusCode == 409) throw Exception(data['detail'] ?? 'Email already registered');
      if (response.statusCode == 422) throw Exception(data['detail'] ?? 'Invalid input data');
  ...
  Future<TokenDto> login({
    ...
    try {
      ...
      if (response.statusCode == 200) return TokenDto.fromJson(data);
      if (response.statusCode == 401) throw Exception(data['detail'] ?? 'Incorrect email or password');
      if (response.statusCode == 404) throw Exception('User not found');
      if (response.statusCode == 422) throw Exception('Invalid input data');
